...
private:
    ...
    // store vertex and index buffer in same vkbuffer
    ...
    void createInstance() {
        ...
        auto reqiredExtensions = getRequiredExtensions();
        createInfo.enabledExtensionCount = static_cast<uint32_t>(reqiredExtensions.size());
        createInfo.ppEnabledExtensionNames = reqiredExtensions.data();
    ...
    VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
        if (capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) {
            ...
            int width, height;
            // WIDTH and HEIGHT most times (not retina display)
  ...
  /*        createInfo.components.r = VK_COMPONENT_SWIZZLE_IDENTITY;
    ...
    void createGraphicsPipeline() {
        ...
        // for non dynamic viewport and scissor state
        ...
        // disables output to framebuffer if true
        ...
        // disabled for now (better than anti aliasing)
    ...
    void recreateSwapChain() {
        // pause window on minimize (causes errors)
        int width = 0, height = 0;
        glfwGetFramebufferSize(window, &width, &height);
